 ...
 */
export const fetchChatGPTResponse = async (code, isExport = false) => {
  const requestData = {
    model: "gpt-4o",
    messages: [
      {
        role: "system",
        content: isExport
          ? "나는 입력 받은 내용의 표 부분만 XML/XHTML 형태로 변환하여 제공한다. 답변은 한국어로 제공하지만 설명은 하지 않고 표만 제공한다."
      ...
      },
      {
        ...
        content: `${
          isExport
            ? code
            : `다음 코드를 분석하고 변수, 함수 및 그 역할에 대한 자세한 설명을 제공하십시오: \n\n${code}`
        }`
      }
    ],
    max_tokens: 4096,
